...
export default function BasicTable() {
    ...
    useEffect(() => {
        ...
        axios.get('http://localhost:1337/api/GetRequest?data=' + JSON.stringify({
            ...
            setseminars(result.data.request)
    ...
    const UpdateRequest = async (semi, index) => {
        ...
        console.log("Get USer Arr => ", getuserarr);
        axios.put('http://localhost:1337/api/UpdateRequest?data=' + JSON.stringify({
            TeacherID: getuserarr._id,
            UserID: semi.UserID
        })).then((result) => {
                ...
                <Table sx={{ minWidth: 65 }} aria-label="simple table">
                    <TableHead>
                        <TableRow sx={{ backgroundColor: "#007FFF", }}>
                            ...
                            <TableCell sx={{ color: "white", marginLeft: "2px" }} align="right">Email</TableCell>
                    ...
                    <TableBody>
                        {
                            seminars.map((semi, index) => (
                                ...
                                >
                                    ...
                                    <TableCell sx={{ marginLeft: "2px" }} align="right">{semi.UserID.email}</TableCell>
                                 
                                    {semi.isRequest === false ?
                                        <TableCell align="right" style={{ display: "flex" }}>
                                            <TableCell sx={{ marginLeft: "2px" }}>  <button onClick={(event) => UpdateRequest(semi, index)}>Accept</button>  </TableCell>
                                            <TableCell sx={{ marginLeft: "2px" }}> <button onClick={(event) => unenrolledHandller(semi, index)}>Reject</button></TableCell>
                                        ...
                                        :
                                        <TableCell align="right" style={{ display: "flex" }}>
                                            <TableCell sx={{ marginLeft: "2px" }}> <button onClick={(event) => unenrolledHandller(semi, index)}>Delete</button></TableCell>
            ...
            <div style={{ marginLeft: "-80px", width: "120%", marginBottom: "10px" }} className="profile-view">
                <div className="profile-body">
                    ...
                    <div className="profile-info">
                        <h3 style={{ color: "#007FFF" }} className="profile-heading">Total Registered</h3>
                        <p className="profile-detail">{count}</p>
                    </div>
