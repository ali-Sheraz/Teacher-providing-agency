...
export default function BasicTable() {
    ...
    useEffect(() => {
        console.log("hello")
        axios.get("http://localhost:1337/api/getReq").then((res) => {
    ...
    const UpdateRequest = async (semi, index) => {
        ...
        console.log("Get USer Arr => ", getuserarr);
        axios.put('http://localhost:1337/api/getappro?id=' + semi._id
        ).then((result) => {
    ...
    const DeleteRequest = async (semi, index) => {
        ...
        console.log(semi._id)
        console.log("Get USer Arr => ", getuserarr);
        axios.delete('http://localhost:1337/api/getdel?id=' + semi._id
        ).then((result) => {
            console.log("response", result.data.request)
            alert("Deleted")
        })
...
<button onClick={suw}>User</button>
            ...
            <TableContainer component={Paper} sx={{ marginLeft: 2, width: "61%", marginLeft: "21%", marginTop: "-57%" }}>
